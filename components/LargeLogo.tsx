...
const LargeLogo: React.FC<LargeLogoProps> = ({ 
  ...
  // If we are generating the background, we might want to fade out a bit, but not completely.
  ...
  return (
    ...
    >
      ...
      <div className={`relative w-full flex justify-center items-center h-0 transition-opacity duration-1000 ${isRedMode ? 'opacity-0 scale-95 pointer-events-none' : 'opacity-100'}`}>
        <div className="absolute animate-float md:translate-x-56 translate-x-24 md:-top-32 -top-24 z-20">
          <div className={`bg-white dark:bg-black border-2 px-6 py-4 rounded-3xl shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] dark:shadow-[8px_8px_0px_0px_rgba(255,255,255,0.1)] transition-[background-color,border-color,shadow,color] duration-500 border-zinc-900 dark:border-white`}>
            <p className={`text-sm md:text-base font-bold tracking-tight italic transition-colors duration-500 text-zinc-900 dark:text-white`}>
          ...
          <div className="absolute -bottom-5 left-8 w-10 h-8">
            <svg viewBox="0 0 40 40" fill="none" className="w-full h-full drop-shadow-[4px_4px_0px_rgba(0,0,0,0.5)]">
              <path 
                ...
                className={`transition-[fill,stroke] duration-500 fill-white dark:fill-black stroke-zinc-900 dark:stroke-white`} 
                strokeWidth="2.5"
              />
            </svg>
          </div>
      ...
      <a 
        ...
        className={`mb-14 group relative z-10 transition-all duration-1000 ${isRedMode ? 'scale-75 opacity-50 grayscale' : 'scale-100 opacity-100'}`}
      >
        ...
        <div className={`relative w-32 h-32 md:w-40 md:h-40 rounded-full overflow-hidden border-2 transition-[border-color,transform,box-shadow] duration-500 group-hover:scale-105 shadow-2xl ${isRedMode ? 'border-red-600' : 'border-zinc-900 dark:border-white'}`}>
          <img 
            src={avatarUrl} 
      ...
      {/* Brand Name Text */}
      <div className={`transition-all duration-1000 ${isRedMode ? 'opacity-20 blur-md pointer-events-none' : 'opacity-90'}`}>
        <div className="flex flex-col">
          <span 
            className="text-7xl md:text-[11rem] tracking-[0.25em] font-weight-transition relative"
            style={{ 
      ...
      {/* ENLARGED: Commission Status - Persistent and Interactive */}
      <div className="flex flex-col items-center mb-12 z-20 pointer-events-auto">
        ...
        <button 
          ...
          className="flex items-center space-x-10 group/status transition-all duration-300 focus:outline-none hover:scale-105 active:scale-95 cursor-pointer"
        >
